...
density = @density_earth;                % put the handle to your density function here
...
for j=1:max_nodes_per_box
    box(:,j) = A*cube_shrink * [j/max_nodes_per_box;  mod(r1*j,1);  mod(r2*j,1)]/N;       % the box is shrunk to account for inwards corner % TODO: can this be vectorized?
    box(:,j) = box(:,j) +  delta;
end    
...
nodes = reshape(repmat(corners_used,max_nodes_per_box,1), dim,[]) + ...
    reshape(repmat(box,1,count), dim, []);                                  % coordinates of the actual nodes
%% remove nodes outside the density support                                                 
[f_vals, ~] = in_domain(nodes(1,:), nodes(2,:), nodes(3,:));             % values of the density function at those nodes
cnf = nodes(:, f_vals);                                                      % after removing nodes with zero density
...
cnf = repel(cnf, k_value, repel_steps, @in_domain, repel_power, density, fileID);
toc 
