...
parfor i=1:N^dim
    ...
    eval_pts = num2cell(bsxfun(@plus, corner, cube_vectors/N),1);
    ...
    nodes(:,:,i) = box;   
    if max(fun_values)>=threshold                                                               % TODO: harde-coded condition
...
bdry_removed_indices = reshape( f_vals > threshold, max_nodes_per_box, []);                     % indices of nodes that have to be removed; hard-coded
